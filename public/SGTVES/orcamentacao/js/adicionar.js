$(document).ready(function () {
  ...
  $("[for=OBJETO]").append('<sup class="col-red">*</sup>');
  ...
  $("#ID_NATUREZA_ORCAMENTACAO").change(function () {
    if ($("#ID_NATUREZA_ORCAMENTACAO").val() === "2") {
      ...
      $(
        "#ANULACAO_ID_ACAO,#ID_NATUREZA_ORIGEM,#ID_DESTINACAO_ORIGEM,#DATA_MOVIMENTACAO"
      )
        .val("")
        .selectpicker("refresh"); //DIV ANULAÇÂO

      $("#REFORCO_ID_ACAO,#ID_NATUREZA_DESTINO,#ID_DESTINACAO_DESTINO")
        .val("")
  ...
  $("#ID_NATUREZA_ORCAMENTACAO").change(function () {
    if ($("#ID_NATUREZA_ORCAMENTACAO").val() === "1") {
      ...
      $("#DIV_VALOR").hide();
      $(
        "#NDD,#ID_ORGAO_ORIGEM,#ID_ORGAO_DESTINO,#ID_UNIDADE_ORCAMENTARIA_ORIGEM,#ID_UNIDADE_ORCAMENTARIA_DEST,#ID_ACAO_ORIGEM,#ID_ACAO_DESTINO,#DATA_DESCENTRALIZACAO"
      )
        .val("")
  ...
  $("#NDD").maskMoney({ decimal: ".", thousands: ".", precision: 3 });


  $("#Adicionar").bootstrapValidator({
  ...
  if (Salvar) {
      ...
      valorAnulacao = 0; jQuery.each(secGrid.getNameValues("ID", [
        "ANULACAO_ID_ACAO",
        "ANULACAO_ID_NATUREZA",
        "ANULACAO_ID_DESTINACAO",
        "ANULACAO_VALOR"
    ]), function () { valorAnulacao += parseFloat(this.ANULACAO_VALOR.replace(/\.|-/gm, '').replace(',', '.')) });

      valorReforco = 0; jQuery.each(secGrid2.getNameValues("ID", [
        "REFORCO_ID_ACAO",
        "REFORCO_ID_NATUREZA",
        "REFORCO_ID_DESTINACAO",
        "REFORCO_VALOR"
    ]), function () { valorReforco += parseFloat(this.REFORCO_VALOR.replace(/\.|-/gm, '').replace(',', '.')) });


      if(valorAnulacao != "0" ){
      if (valorReforco != valorAnulacao) {

        swal({
          title: "Erro",
          text: "Valor Total de Reforço deve ser igual ao valor total de Anulação.",
          type: "error",
